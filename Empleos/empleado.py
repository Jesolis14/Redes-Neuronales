...
dataset1 = pd.read_csv('employee_promotion.csv')
...
def build_model(hp):
    ...
    optimizers_dict = {
        ...
        "SGD": SGD(learning_rate=lr, momentum=hp.Float('momentum', 0.0, 0.9, 0.1), clipvalue=1.0),
...
tuner = kt.Hyperband(
    ...
    factor               = 3,
    directory            = 'salida',
    project_name         = 'Empleados_Under',
    overwrite            = True
)

early_stop = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
tuner.search(X_resample, y_resample, validation_split=0.2, verbose=0, callbacks=[early_stop])
...
y_resample = np.array(y_resample, dtype=np.float32)

historial = mi_mejor_modelo.fit(X_resample, y_resample,validation_split=0.2,  epochs=50,  verbose=1 )
...
tuner = kt.Hyperband(
    ...
    factor               = 3,
    directory            = 'salida',
    project_name         = 'Empleos_Weight',
    overwrite            = True
)

early_stop = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
hist = tuner.search(X_train, y_train, validation_split=0.2, class_weight=class_weights , callbacks=[early_stop], verbose=0)
...
historial = mi_mejor_modelo.fit(X_train, y_train,validation_split=0.2, class_weight=class_weights ,epochs=50,  verbose=1 )
